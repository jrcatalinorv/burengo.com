...
switch($sb){
	case 1: 
		...
		AND p.bgo_status = 1 AND bgo_subcat = 1 ORDER BY p.bgo_stdesc DESC LIMIT ".$offset.", ".$no_of_records_per_page."");			
	  break;
	...
	case 2: 
		...
		INNER JOIN bgo_places pl ON p.bgo_lugar = pl.pcid 
		INNER JOIN bgo_currency cu ON p.bgo_currency = cu.cur_id		
		AND p.bgo_status = 1 AND bgo_subcat = 2 ORDER BY p.bgo_stdesc DESC LIMIT ".$offset.", ".$no_of_records_per_page."");		
	  break;
	...
	default:
			...
			INNER JOIN bgo_places pl ON p.bgo_lugar = pl.pcid 
			INNER JOIN bgo_currency cu ON p.bgo_currency = cu.cur_id
			AND p.bgo_status = 1 ORDER BY p.bgo_stdesc DESC LIMIT ".$offset.", ".$no_of_records_per_page."");	
		break;
 }
